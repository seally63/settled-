...
// ============================================================================
// Trade Profile Approvals
// ============================================================================

/**
 * Get all trades with pending approval status
 * @returns {Promise<object[]>} - Array of pending trade profiles
 */
export async function getPendingTradeApprovals() {
  const { data, error } = await supabase
    .from("profiles")
    .select("id, full_name, business_name, trade_title, phone, created_at, base_postcode, town_city, approval_status")
    .eq("role", "trades")
...
/**
 * Approve a trade profile (makes them visible to clients)
 * @param {string} profileId - Trade profile ID
 * @returns {Promise<object>}
 */
export async function approveTradeProfile(profileId) {
...
/**
 * Reject a trade profile
 * @param {string} profileId - Trade profile ID
 * @returns {Promise<object>}
 */
export async function rejectTradeProfile(profileId) {
  if (!profileId) throw new Error("Profile ID is required");
