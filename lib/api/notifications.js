...
export function handleNotificationNavigation(notification, router) {
  ...
  switch (type) {
    ...
    // Client-targeted notifications. Settled mobile is now trade-
    // only — these payloads still get sent to client devices via
    // the web directory's push channels, but on the mobile app a
    // trade would never receive them. If somehow one slips through
    // (test push, account-switch edge case), fall back to /messages
    // rather than try to navigate into the deleted /client/* routes.
    ...
    // Work-completed is also client-targeted (trades see their own
    // mark-complete via /quotes/[id] navigation). Same fallback.
    case "work_completed":
      router.push("/messages");
      break;
