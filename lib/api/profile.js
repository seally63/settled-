...
export async function updateServiceRadius(km) {
  console.log("[DEBUG profile.js] updateServiceRadius called with km:", km);
  const uid = await getUserId();
  console.log("[DEBUG profile.js] uid:", uid);
  if (!uid) throw new Error("Not signed in");

  const v = Number.isFinite(Number(km)) ? Number(km) : 25;
  console.log("[DEBUG profile.js] Calling RPC rpc_update_service_radius with p_km:", v);
  const { data, error } = await supabase.rpc("rpc_update_service_radius", { p_km: v });
  console.log("[DEBUG profile.js] RPC response - data:", data, "error:", error);
  if (error) {
    console.log("[DEBUG profile.js] RPC error details:", JSON.stringify(error, null, 2));
    throw error;
  }
  return data ?? true;
}
