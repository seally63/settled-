 ...
 * Persists: requester_id, status='open', details, suggested_title, budget_band, job_outcode
...
export async function requestDirectQuote(tradeId, opts = {}) {
  ...
  const job_outcode = (opts?.job_outcode || "").toUpperCase() || null; // e.g., G3, G14, EH1
  ...
  if (reqErr) {
    ...
    if (msg.includes("job_outcode")) {
      throw new Error("Invalid postcode area.");
    }
    throw reqErr;
  }
