...
export async function requestDirectQuote(tradeId, opts = {}) {
  ...
  // V2 wizard sends foreign-key IDs — persist every one we have a column
  // for so the trade inbox can show the proper service / category name
  // and icon directly from the join (no more guessing via parseTitle).
  //
  // quote_requests columns: service_type_id, category_id (the latter was
  // renamed from service_category_id in a later migration, see
  // supabase/migrations/20260122_security_definer_fixes.sql et al which
  // all join on qr.category_id). property_type_id / timing_option_id
  // aren't in the schema yet — leave them out until migrated.
  ...
  if (category_id) insertRow.category_id = category_id;
