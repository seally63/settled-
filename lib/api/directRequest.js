...
export async function requestDirectQuote(tradeId, opts = {}) {
  ...
  const outsideServiceArea = opts?.outsideServiceArea ?? false; // Flag if client knowingly requests outside trade's area
  const distanceMiles = opts?.distanceMiles ?? null; // Distance in miles when outside service area

  // 1) create the quote request (marked as direct since client is requesting a specific trade)
    ...
    .insert([
      {
        ...
        budget_band,
        postcode,
        is_direct: true, // Mark as direct request since client is requesting a specific trade
      },
    ])
    .select("id")
    .single();
