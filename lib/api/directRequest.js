...
export async function requestDirectQuote(tradeId, opts = {}) {
  ...
  //
  // quote_requests currently has: service_type_id, service_category_id
  // (per 20250101 initial schema). property_type_id / timing_option_id
  // aren't in the schema yet — leave them out until migrated.
  const service_type_id = opts?.service_type_id ?? null;
  const service_category_id = opts?.category_id ?? null;
  ...
  if (service_category_id) insertRow.service_category_id = service_category_id;
