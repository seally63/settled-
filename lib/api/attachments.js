 ...
 * ============================================================
 *
 * Mirrors the request-attachment pipeline above so review photos
 * use the same upload "system" as request photos:
 *   · same normaliseLocalItem + toArrayBuffer flow
 *   · same `{root}/{uuid}.{ext}` object-key shape under a per-quote
 *     folder (matches the existing `reviews/...` storage policy
 *     that gates the `review-photos` bucket)
 *   · same progress callback contract (done, total)
 *
 * The `review-photos` bucket is public (per the schema-unify
 * migration), so the helper returns both the raw object paths and
 * the resulting public URLs. `rpc_submit_review.p_photos` currently
 * accepts URLs; if it ever moves to paths + signed URLs the call
 * site swaps which array it forwards without changing the upload
 * pipeline itself.
 * ============================================================ */

const REVIEW_BUCKET = "review-photos";

/** Build an object key inside a quote's review folder. */
export function makeReviewObjectPath(quoteId, ext = "jpg") {
  const safeExt = String(ext || "jpg").replace(/^\./, "");
  return `reviews/${quoteId}/${uuid4()}.${safeExt}`;
}

/** Bucket existence check for review-photos — same shape as
 *  ensureBucketExists() above, scoped to the review bucket. */
async function ensureReviewBucketExists() {
  const { error } = await supabase.storage.from(REVIEW_BUCKET).list("", { limit: 1 });
  if (error && /not found|does not exist/i.test(error.message || "")) {
    throw new Error(
      `Storage bucket "${REVIEW_BUCKET}" not found. ` +
        `Create it in Supabase or run the reviews_schema_unify migration.`
    );
  }
}

/**
 * Upload a single review image and return the public URL. Mirrors
 * `uploadTempImage` for the request-form flow: single-photo entry
 * point with milestone progress callbacks (10/30/100), so the
 * leave-review screen can show inline per-thumbnail progress
 * exactly like the request-form photo strip.
 *
 * @param {string} quoteId
 * @param {string|{uri:string,base64?:string}} localItem
 * @param {(progress:number)=>void} [onProgress] - 0..100
 * @returns {Promise<{success:boolean, path?:string, url?:string, error?:string}>}
 */
export async function uploadSingleReviewImage(quoteId, localItem, onProgress) {
  if (!quoteId || !localItem) {
    return { success: false, error: "Missing quoteId or image" };
  }

  try {
    await ensureReviewBucketExists();
  } catch (e) {
    return { success: false, error: e.message };
  }
  ...
  const objectPath = makeReviewObjectPath(quoteId, meta.ext);
  ...
  try {
    ...
    if (error) {
      console.warn(
        "uploadSingleReviewImage failed:",
        objectPath,
        error.message || error
      );
      return { success: false, error: error.message || "Upload failed" };
    ...
    let url = null;
    try {
      url = supabase.storage.from(REVIEW_BUCKET).getPublicUrl(objectPath)?.data
        ?.publicUrl;
    } catch {}

    return { success: true, path: objectPath, url };
  } catch (err) {
    console.warn(
      "uploadSingleReviewImage exception:",
      objectPath,
...
/**
 * Upload an array of local images for a review and return both the
 * object paths and the resulting public URLs.
 *
 * Symmetrical with uploadRequestImages — anyone who wants the same
 * progress UX or has a list of local items in the same shape can
 * call either function and expect the same behaviour.
 *
 * @param {string} quoteId
 * @param {Array<string|{uri:string,base64?:string}>} localItems
 * @param {(done:number,total:number)=>void} [onProgress]
...
export async function uploadReviewImages(
  ...
  if (!quoteId || !localItems?.length) return { paths: [], urls: [] };
  ...
  // Convert paths to public URLs. `rpc_submit_review.p_photos`
  // currently stores URLs in `reviews.photos`, so we need both for
  // back-compat. If the schema migrates to storing paths and signing
  // on read, the caller can drop the URL list and forward `paths`.
  const urls = paths
    .map((p) => {
      try {
        return supabase.storage.from(REVIEW_BUCKET).getPublicUrl(p)?.data
          ?.publicUrl;
      } catch {
        return null;
      }
