  ...
  try {
    ...
    const enrichedRequests = await Promise.all(
      (requests || []).map(async (req) => {
        ...
        // Get request targets with trade info for direct requests
        const { data: targetsData } = await supabase
          .from("request_targets")
          .select("id, trade_id, invited_by, profiles:trade_id(business_name, full_name)")
          .eq("request_id", req.id);

        const targetsCount = (targetsData || []).length;

        // For direct requests, get the trade business name
        let tradeBusinessName = null;
        // Also check if any target was invited by client as fallback for is_direct
        const hasClientInvitedTarget = (targetsData || []).some(t => t.invited_by === "client");
        const isDirectRequest = req.is_direct === true || hasClientInvitedTarget;

        if (isDirectRequest && targetsData && targetsData.length > 0) {
          const clientInvitedTarget = targetsData.find(t => t.invited_by === "client");
          const targetToUse = clientInvitedTarget || targetsData[0];
          tradeBusinessName = targetToUse?.profiles?.business_name || targetToUse?.profiles?.full_name || null;
        }
...
export async function removeRecentSearch(term) {
  ...
  try {
    ...
    // Filter out the term (case-insensitive)
    const filtered = existing.filter(
      (s) => s.toLowerCase() !== trimmed.toLowerCase()
    );

    await SecureStore.setItemAsync(RECENT_SEARCHES_KEY, JSON.stringify(filtered));
  } catch (e) {
    console.warn("removeRecentSearch error:", e.message);
  }
}
...
export async function getUserFirstName(userId) {
  ...
  try {
    ...
    if (error) {
      console.warn("getUserFirstName query error:", error.message);
      return null;
    }
